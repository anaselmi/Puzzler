...
class RenderProcessor(esper.Processor):
    ...
    def process(self):
        ...
        self.window.clear()
        unrendered = list(self.world.get_components(Renderable, Positionable))
        priorities = [1, 2, 3, 4, 5]
        for priority in priorities:
            for ent, (rend, pos) in self.world.get_components(Renderable, Positionable):
                print(pos.x, pos.y, "RENDER")
                if not rend.active:
                    continue
                if rend.priority == priority:
                    print(pos.x, pos.y)
                    self.window.draw_char(pos.x, pos.y, rend.char, fg=rend.fg)


class Renderable:
    def __init__(self, char, fg=WHITE, active=True, priority=1):
        self.char = char
        self.fg = fg
        self.active = active
